...
class TextGrapher:
    def __init__(self):
        lFont =  os.listdir('fonts/')
        self._fonts = enumerate(lFont)

        lDirection = ['rtl', 'utd']
        self._direction = enumerate(lDirection)

        lNoise = ['none', 'saltAndPeper']
        self._noise = enumerate(lNoise)
    ...
    def __selectRandomColor(self):
        self._color = (int(random.random() * 255),int( random.random() * 255),int( random.random() * 255) , 255)
        return self._color

    def __selectRandomFont(self):
        return random.choice(list(self._fonts))

    def __selectRandomNoise(self):
        return random.choice(list(self._noise))

    def __selectRandomDirection(self):
        return random.choice(list(self._direction))
    ...
    def produceTextImage(self, text = "سلام"):
        ...
        unicodeFont = ImageFont.truetype(f"./fonts/{self.__selectRandomFont()[1]}", size)
        
        draw.text((100,100), wrapped, spacing=30, font = unicodeFont, fill = fontColor, direction = 'rtl', align='left')
